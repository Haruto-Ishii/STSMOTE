...
from synthesis_method.TabSyn.tabsyn.vae.model import Model_VAE, Encoder_model, Decoder_model
from synthesis_method.TabSyn.utils_train import preprocess, TabularDataset
...
def main(X_minority, y_minority, X_majority, y_majority, num_to_generate, info, random_state):
    ...
    X_num, X_cat, categories, d_numerical, num_inverse, cat_inverse = preprocess(X_minority, y_minority, info, task_type = info['task_type'], inverse=True)
    ...
    with torch.no_grad():
        ...
        train_z = pre_encoder(X_train_num, X_train_cat)
        
    return pre_decoder, train_z.float(), num_inverse, cat_inverse
