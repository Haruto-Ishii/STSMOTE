...
def main(X_minority, y_minority, X_majority, y_majority, num_to_generate, info, random_state):
    """
    Safe-Tube SMOTE (Proposed Method)
    
    数値特徴量空間で幾何学的な安全領域(Tube)を計算してノイズを加え、
    カテゴリカル特徴量は近い方の親データから継承する。
    """
    ...
    def generate_sphere(center_idx, n_gen):
        ...
        rs = rng.random(n_gen) ** (1/n_dim_num) * radius
    ...
    if num_tube > 0 and n_pairs > 0:
        ...
        for p_idx, (idx_a, idx_b) in enumerate(pairs):
            ...
            noise_mags = rng.random(n_gen_target) ** (1/n_dim_num) * final_h_max * 1.0
    ...
    if len(generated_num_data) > 0:
        ...
        X_min_num_raw = X_minority[:, num_indices]
        ptp = np.ptp(X_min_num_raw, axis=0)
        constant_mask = (ptp < 1e-9)
        
        if np.any(constant_mask):
            constant_values = X_min_num_raw[0, constant_mask]
            X_gen_num[:, constant_mask] = constant_values
